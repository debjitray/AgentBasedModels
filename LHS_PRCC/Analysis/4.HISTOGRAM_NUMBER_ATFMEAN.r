...
for (i in 1:9) {
	...
	pdf(file=paste('Z.Number_ATFMEAN/1.PRCC_',TIMEFRAME[i],'.pdf',sep=""),width=200,height=100,pointsize=50);
	lcex = 0.5;
	...
	pdf(file=paste('Z.Number_ATFMEAN/1.SIGNS_',TIMEFRAME[i],'.pdf',sep=""),width=200,height=100,pointsize=50);
	lcex = 0.5;
	par(mfrow=c(2,5),mar=c(50, 5, 4, 1));
	SIGNIFICANTS=c();
	for (joop in 1:11) {
	...
	M=as.matrix(z);
	pdf(file=paste('Z.Number_ATFMEAN/2.Number_HIST_',TIMEFRAME[i],'.pdf',sep=""),width=100,height=100,pointsize=200);
	lcex = 0.5;
