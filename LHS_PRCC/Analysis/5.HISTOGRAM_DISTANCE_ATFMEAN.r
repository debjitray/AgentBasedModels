...
CELL=c('Undifferentiated spermatogonia','Differentiated spermatogonia','Preleptotene','Leptotene','Zygotene','Pachytene','Diplotene','Secondary spermatocyte','Round spermatid','Elongated spermatid');
...
for (i in 1:9) {
	...
	pdf(file=paste('Z.Distance_ATFMEAN/1.PRCC_',TIMEFRAME[i],'.pdf',sep=""),width=200,height=100,pointsize=50);
	lcex = 0.5;
	...
	pdf(file=paste('Z.Distance_ATFMEAN/1.SIGNS_',TIMEFRAME[i],'.pdf',sep=""),width=200,height=100,pointsize=50);
	lcex = 0.5;
	par(mfrow=c(2,5),mar=c(50, 5, 4, 1));
	SIGNIFICANTS=c();
	for (joop in 1:10) {
	...
	M=as.matrix(z);
	pdf(file=paste('Z.Distance_ATFMEAN/2.Distance_HIST_',TIMEFRAME[i],'.pdf',sep=""),width=100,height=100,pointsize=200);
	lcex = 0.5;
