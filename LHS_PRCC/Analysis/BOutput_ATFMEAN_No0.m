...
for set=0:RUNS-1  
    set
    % FOR UNDIFF
    c1=['/Users/debjit/Documents/ABM/6.Final model/New/LHS_TEST/SET_',num2str(set),'/Results_Stats/0.Output_2/1.Undiff.txt'];
    W1=importdata(c1);
    UNDIFF_DIST=[UNDIFF_DIST;W1.data(:,3)'];
    % FOR DIFF
    c2=['/Users/debjit/Documents/ABM/6.Final model/New/LHS_TEST/SET_',num2str(set),'/Results_Stats/0.Output_2/2.Diff.txt'];
    W2=importdata(c2);
    DIFF_DIST=[DIFF_DIST;W2.data(:,3)'];
    % FOR PRELEP
    c3=['/Users/debjit/Documents/ABM/6.Final model/New/LHS_TEST/SET_',num2str(set),'/Results_Stats/0.Output_2/3.Prelep.txt'];
    W3=importdata(c3);
    PRELEP_DIST=[PRELEP_DIST;W3.data(:,3)'];
    % FOR LEP
    c4=['/Users/debjit/Documents/ABM/6.Final model/New/LHS_TEST/SET_',num2str(set),'/Results_Stats/0.Output_2/4.Lep.txt'];
    W4=importdata(c4);
    LEP_DIST=[LEP_DIST;W4.data(:,3)'];
    % FOR ZYG
    c5=['/Users/debjit/Documents/ABM/6.Final model/New/LHS_TEST/SET_',num2str(set),'/Results_Stats/0.Output_2/5.Zyg.txt'];
    W5=importdata(c5);
    ZYG_DIST=[ZYG_DIST;W5.data(:,3)'];
    % FOR PACHY
    c6=['/Users/debjit/Documents/ABM/6.Final model/New/LHS_TEST/SET_',num2str(set),'/Results_Stats/0.Output_2/6.Pachy.txt'];
    W6=importdata(c6);
    PACHY_DIST=[PACHY_DIST;W6.data(:,3)'];
    % FOR DIP
    c7=['/Users/debjit/Documents/ABM/6.Final model/New/LHS_TEST/SET_',num2str(set),'/Results_Stats/0.Output_2/7.Dip.txt'];
    W7=importdata(c7);
    DIP_DIST=[DIP_DIST;W7.data(:,3)'];
    % FOR SEC
    c8=['/Users/debjit/Documents/ABM/6.Final model/New/LHS_TEST/SET_',num2str(set),'/Results_Stats/0.Output_2/8.Sec.txt'];
    W8=importdata(c8);
    SEC_DIST=[SEC_DIST;W8.data(:,3)'];
    % FOR ROUND
    c9=['/Users/debjit/Documents/ABM/6.Final model/New/LHS_TEST/SET_',num2str(set),'/Results_Stats/0.Output_2/9.Round.txt'];
    W9=importdata(c9);
    ROUND_DIST=[ROUND_DIST;W9.data(:,3)'];
    % FOR ELONG
    c10=['/Users/debjit/Documents/ABM/6.Final model/New/LHS_TEST/SET_',num2str(set),'/Results_Stats/0.Output_2/10.Elong.txt'];
    W10=importdata(c10);
    ELONG_DIST=[ELONG_DIST;W10.data(:,3)'];

    
    cS=['/Users/debjit/Documents/ABM/6.Final model/New/LHS_TEST/SET_',num2str(set),'/Results_Errors/Y.Final_All_PRCC.csv'];
    WS=importdata(cS);
    WS=WS(:,2);
    KOLA=zeros(10,1);
    KOLA(find(WS))=WS(find(WS));
    KOLA=KOLA(2:10);
    SPERM_DIST=[SPERM_DIST;KOLA'];
...
for MAIN=round([4.01:8.61:72.89]*24)
	...
	CUMM_UNDIFF(CUMM_UNDIFF==0) =nan;
    CUMM_DIFF(CUMM_DIFF==0) =nan;
	CUMM_PRELEP(CUMM_PRELEP==0) =nan;
	CUMM_LEP(CUMM_LEP==0) =nan;
	CUMM_ZYG(CUMM_ZYG==0) =nan;
	CUMM_PACHY(CUMM_PACHY==0) =nan;
	CUMM_DIP(CUMM_DIP==0) =nan;
	CUMM_SEC(CUMM_SEC==0) =nan;
	CUMM_ROUND(CUMM_ROUND==0) =nan;
	CUMM_ELONG(CUMM_ELONG==0) =nan;
    ...
    Names={'UNDIFFERENTIATED_SPERMATOGONIA' 'DIFFERENTIATED_SPERMATOGONIA' 'PRELEPTOTENE' 'LEPTOTENE' 'ZYGOTENE' 'PACHYTENE' 'DIPLOTENE' 'SECOND_SPERMATOCTYE' 'ROUND_SPERMATIDS' 'ELONGATED_SPERMATIDS' 'SPERM'}';
    ...
    % SIGN DATA FOR 11 CELL TYPES
    sign=[sign1' sign2' sign3' sign4' sign5' sign6' sign7' sign8' sign9' sign10' sign11']';
    sign=num2cell(sign);
    k=repmat('%.3f\t',1,RUNS);
    fileName = ['Z.Number_ATFMEAN/Y.SIGNS_' num2str(time) '.txt']; 
    fid=fopen(fileName,'w');
    for row =1:length(Names)
    ...
    for i=1:11
        ...
        %dlmwrite(['Z.Number_ATFMEAN/Z.Summ_' num2str(i) '_' num2str(time) '.txt'],eval(INDEX),'delimiter', '\t');
        %dlmwrite(['Z.Number_ATFMEAN/Z.Summ_' num2str(i) '_' num2str(time) '.txt'],eval(SIGN_VALUES),'-append','delimiter', '\t','precision', 2);
        %dlmwrite(['Z.Number_ATFMEAN/Z.Summ_' num2str(i) '_' num2str(time) '.txt'],eval(PRCC_VALUES),'-append','delimiter', '\t','precision', 2);
        %dlmwrite(['Z.Number_ATFMEAN/Z.Summ_' num2str(i) '_' num2str(time) '.txt'],eval(LABELS)','-append','delimiter', '');
    ...
    if (MAIN==1129) 
        ...
        PRCC_PLOT(LHS_FINAL(find(~isnan(SPERM)),:),SPERM(find(~isnan(SPERM))),1129,PRCC_vari,'SPERM');
    end
end
...
dlmwrite('Z.Number_ATFMEAN/0.VALIDSIZE.csv',VALIDSIZE);
