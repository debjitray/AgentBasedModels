...
for MAIN=round([4.01:8.61:72.89]*24)
    ...
    MEAN_UNDIFF=sum(CUMM_UNDIFF)';
    MEAN_DIFF=sum(CUMM_DIFF)';
    MEAN_PRELEP=sum(CUMM_PRELEP)';
    MEAN_LEP=sum(CUMM_LEP)';
    MEAN_ZYG=sum(CUMM_ZYG)';
    MEAN_PACHY=sum(CUMM_PACHY)';
    MEAN_DIP=sum(CUMM_DIP)';
    MEAN_SEC=sum(CUMM_SEC)';
    MEAN_ROUND=sum(CUMM_ROUND)';
    MEAN_ELONG=sum(CUMM_ELONG)';
    ...
    prcc=num2cell(prcc);
    k=repmat('%.3f\t',1,RUNS);
    fileName = ['Z.Number_ATFSUM/X.PRCC_' num2str(time) '.txt'];
    ...
    sign=num2cell(sign);
    k=repmat('%.3f\t',1,RUNS);
    fileName = ['Z.Number_ATFSUM/Y.SIGNS_' num2str(time) '.txt']; 
    fid=fopen(fileName,'w');
    for row =1:length(Names)
    ...
    for i=1:11
        ...
        %dlmwrite(['Z.Number_ATFSUM/Z.Summ_' num2str(i) '_' num2str(time) '.txt'],eval(INDEX),'delimiter', '\t');
        %dlmwrite(['Z.Number_ATFSUM/Z.Summ_' num2str(i) '_' num2str(time) '.txt'],eval(SIGN_VALUES),'-append','delimiter', '\t','precision', 2);
        %dlmwrite(['Z.Number_ATFSUM/Z.Summ_' num2str(i) '_' num2str(time) '.txt'],eval(PRCC_VALUES),'-append','delimiter', '\t','precision', 2);
        %dlmwrite(['Z.Number_ATFSUM/Z.Summ_' num2str(i) '_' num2str(time) '.txt'],eval(LABELS)','-append','delimiter', '');
...
dlmwrite('Z.Number_ATFSUM/0.VALIDSIZE.csv',VALIDSIZE);
